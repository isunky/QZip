...
轻压 · QZip 采用成熟的 7-Zip 控制台能力作为首个压缩后端，通过独立的后端适配层、任务系统和现代桌面界面，形成一款强调轻量、简洁、可靠和跨平台一致性的开源压缩工具。

## 1.2 产品定位

> 轻压 · QZip 是一款以拖拽、智能默认配置和现代任务流为核心的跨平台压缩软件。
...
1. 不将 CLI 命名为单独的 `qzip`，避免与 Linux 生态已有命令产生冲突；
...
QZip-{version}-macos-arm64.dmg
QZip-{version}-macos-x64.dmg
QZip-{version}-linux-x86_64.AppImage
